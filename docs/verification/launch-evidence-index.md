...
- Current boundary note: the publication-safe verdict still stands. The 2026-04-08 update closes two of the four remaining active blockers: tenant-scoped object isolation is now enforced via x-tenant-id across all case/report/export/artifact routes with cross-tenant 403, and reviewer-scoped mutation authorization is now enforced via assignedReviewerId matching on review/finalize. The remaining gaps for any stronger security or deployment claim are: x-tenant-id relies on a plain header rather than a cryptographically signed tenant token and there is still remaining provenance weakness around caller-supplied `volumeDownloadUrl` despite the worker-side origin guard. Hosted runtime evidence is now current for the public head.
- Latest hosted-validated head: `3f42a4b8d3f912f9eb84ca0f6bf3e1d56f932170`
- Current local validation snapshot above that hosted head: the latest 2026-04-09 rerun is green on `3f42a4b8d3f912f9eb84ca0f6bf3e1d56f932170` with clean editor diagnostics on `src` and `tests`, `npm run build`, and `npm test` at 177 total tests, 176 passing, 0 failing, and 1 skipped. The current local snapshot includes tenant-scoped object isolation (x-tenant-id), reviewer-scoped mutation authorization, try/catch hardening on list and operations-summary routes, the reviewer-role allowlist for review/finalize mutations, fail-closed internal/operator route auth outside development, a cleaned `dist` runtime set aligned to current `src`, and the worker-side origin guard that rejects disallowed absolute `volumeDownloadUrl` fetches before network egress while preserving the bounded voxel-backed seam for same-origin or local-proof paths. The env-gated `verify:postgres-bootstrap` probe still fails closed without `MRI_CASE_STORE_DATABASE_URL` or `DATABASE_URL`, consistent with `docs/verification/postgres-bootstrap-audit-2026-03-27.md` rather than with a runtime regression.
...
- Wave 5 evidence status: locally closed; clinical evidence program complete — reader-study protocol, subgroup analysis plan, PMS activation criteria, and release-linked validation packet created. The current local validation packet now records 177 total tests, 176 pass, 0 fail, and 1 skipped without changing the conservative publication verdict.
