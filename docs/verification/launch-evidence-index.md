...
- Current boundary note: the publication-safe verdict still stands, but the 2026-04-03 revalidation now narrows the active blockers for any stronger security or deployment claim to four areas: shallow actor/object authorization on case/report/export/artifact surfaces, missing relationship-based or case-scoped reviewer authorization beyond the new reviewer-role allowlist, remaining provenance weakness around caller-supplied `volumeDownloadUrl` despite the new worker-side origin guard, and hosted-evidence lag above the current local head. Reviewer identity is now JWT-derived, review/finalize mutations now enforce an explicit reviewer-role allowlist, internal and operator routes now fail closed outside development when auth secrets are unset, the local `dist` runtime set is now cleanly aligned to current `src`, public finalize no longer accepts `deliveryOutcome` overrides, and the worker now rejects disallowed absolute volume origins before network fetch.
- Latest hosted-validated head: `04cb0a57d1e64f8a5cf03a22b4a5c60d37dffc3a`
- Current local validation snapshot above that hosted head: the latest 2026-04-03 rerun on `aef1acba8f96ba489af912743a72fda55e00f2ac` is green with clean editor diagnostics on `src` and `tests`, `npm run build`, and `npm test` at 166 total tests, 165 passing, 0 failing, and 1 skipped. The current local snapshot also includes the reviewer-role allowlist for review/finalize mutations, fail-closed internal/operator route auth outside development, a cleaned `dist` runtime set aligned to current `src`, and the worker-side origin guard that rejects disallowed absolute `volumeDownloadUrl` fetches before network egress while preserving the bounded voxel-backed seam for same-origin or local-proof paths.
...
- Wave 5 evidence status: locally closed; clinical evidence program complete — reader-study protocol, subgroup analysis plan, PMS activation criteria, and release-linked validation packet created. The current local validation packet now records 166 total tests, 165 pass, 0 fail, and 1 skipped without changing the conservative publication verdict.
