...
The inference callback no longer emits a fixed synthetic draft. When no real volume locator is present it still emits a metadata-derived draft that uses the execution contract and series metadata only. When `studyContext.series[].volumeDownloadUrl` is present, the worker only permits relative URLs, same-origin absolute URLs to `MRI_API_BASE_URL`, or loopback-local URLs for the bounded test path; disallowed absolute origins are rejected before fetch and downgraded into a classified metadata fallback. Allowed bounded inputs are then downloaded as one NIfTI volume, parsed without heavyweight dependencies, and used to emit voxel-level measurements plus an SVG slice preview. This remains a narrow Wave 2B seam, not a production imaging runtime or a DICOM-derived pipeline.
